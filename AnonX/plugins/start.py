        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[قناة السورس](https://t.me/s_o_679) ** ᴩʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs**\n\n"
                    else:
...
‹ تم البحث بواسطة : {config.MUSIC_BOT_NAME} ›"""
        ...
        if config.START_IMG_URL:
            try:
                ...
                user = message.from_user.id
                if int(user) == dev_owner: 
                   return await message.reply(f"**𖢿 | : مرحبا حبيبتي فايروس مطورة السورس{message.from_user.mention}\n𖢿 | : كل اقسام التحكم بالبوتات\n𖢿 | : تستطيع التحكم بكل البوتات عن طريق هذه الازرار**",reply_markup=OwnerM)
                if message.from_user.id in owner: 
                   return await message.reply_text(f"**𖢿 | : مرحبا عزيزتي المطورة الاساسي {message.from_user.mention}\n𖢿 | : اليك ازرار التحكم بالاقسام\n𖢿 | : تستطيع التحكم بجميع الاقسام فقط اضغط على القسم الذي تريده**",reply_markup=main_dev_key)
                else:  
